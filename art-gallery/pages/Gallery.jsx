...
const Gallery = ({ onNavigateToRegister }) => {
  ...
  // 1. Fetch data. Default to empty array if data is undefined
  ...
  // DEBUG: Check your console to see if data is actually arriving
  console.log("Artworks from API:", artworks);
  ...
  const getUniqueCategories = () => {
    if (!artworks) return ['all'];
    const categories = artworks.map(a => a.category).filter(Boolean);
    ...
    <div className="flex-1 bg-[#f3f4f6] w-full min-h-screen">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-10 text-red-500">
          <p>Error loading artworks. Please check your connection.</p>
        </div>
      ) : (
        <>
          {/* Category Filter */}
            ...
            <div className="flex flex-row flex-wrap gap-2.5">
              {getUniqueCategories().map(category => (
                <button
                  ...
                  className={`py-2 px-3.5 rounded-full text-[14px] transition-colors duration-200 ${
                    filter === category
          ...
          {filteredArtworks.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-center px-4">
              ...
              <p className="text-[14px] text-[#6b7280]">
                {searchQuery.trim() !== '' ? 'Try a different name.' : 'The gallery is currently empty.'}
              </p>
            </div>
          ) : (
            <div className="flex flex-row flex-wrap justify-center md:justify-start gap-[5%] px-4 pb-10">
              {filteredArtworks.map(artwork => (
                <div key={artwork.id} className="w-full md:w-[30%] bg-white rounded-[14px] mb-6 overflow-hidden shadow-[0_4px_10px_rgba(0,0,0,0.08)]">
                  <img
                    // FIXED: Changed imagesUrls to imageUrls
                    src={artwork.imageUrls?.[0] || 'https://via.placeholder.com/400x300?text=No+Image'} 
                    className="w-full h-45 md:h-55 bg-[#e5e7eb] object-cover"
                    alt={artwork.name}
                  />
                  <div className="p-4">
                    <h3 className="text-[18px] font-semibold mb-1">{artwork.name}</h3>
                    <p className="text-[14px] text-[#6b7280] mb-2">Painted by {artwork.artistName || 'Unknown Artist'}</p>
                    <p className="text-[14px] text-[#374151] mb-3 line-clamp-2">{artwork.description}</p>
                    {/* Added Naira Symbol */}
                    <p className="text-[16px] font-bold mb-3">₦{artwork.price?.toLocaleString()}</p>

                    <button
                      className={`w-full py-2.5 rounded-lg flex items-center justify-center font-semibold text-[14px] transition-colors duration-200 ${
                        orderSuccess && selectedArtwork?.id === artwork.id
